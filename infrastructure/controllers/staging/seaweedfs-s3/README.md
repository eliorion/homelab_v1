...
1. `buckets/<b>.enc.yaml`: copy `advisor-corpus.enc.yaml`'s shape, keys `<B>_S3_ACCESS_KEY` /
   `<B>_S3_SECRET_KEY` (`<B>` = the bucket name upper-cased, `-` -> `_`), values `openssl rand -hex 12` /
...
3. `clusters/staging/infrastructure.yaml`: `- {kind: Secret, name: s3-<b>}` under the config unit's
   `substituteFrom`.
4. `../seaweedfs-config/configure.sh`: `s3.bucket.create -name <b>`.
