...
for N in $STEPS; do
  ...
  k -n azuracast-loadtest scale deploy/listener-sim-site-b deploy/listener-sim-site-c --replicas=1 >/dev/null
  k -n azuracast-loadtest rollout status deploy/listener-sim-site-b --timeout=180s >/dev/null
  k -n azuracast-loadtest rollout status deploy/listener-sim-site-c --timeout=180s >/dev/null
  ...
  VERDICT=ok
  # Both sites short means the shared leg — the uplink — is the limit. One site
  # short means that site's own connection is, and the uplink has more to give.
  if   [ "${B:-0}" -lt "$FLOOR" ] && [ "${C:-0}" -lt "$FLOOR" ]; then VERDICT=uplink-saturated
  elif [ "${B:-0}" -lt "$FLOOR" ] || [ "${C:-0}" -lt "$FLOOR" ]; then VERDICT=one-site-limited
  fi

  printf '%s,%s,%s,%s,%s,%s,%.4f,%.1f,%s\n' \
    "$N" "$((N * 2))" "$B" "$C" "$RB" "$RC" "${CPU:-0}" \
    "$(echo "${TX:-0}" | awk '{print $1/1024}')" "$VERDICT" | tee -a "$OUT"

  if [ "$VERDICT" = "uplink-saturated" ]; then
    echo "===> ceiling found at $((N * 2)) concurrent listeners — stopping"
    break
  fi
done
